...
public class MovieControllerTest {
	...
	public void findById_MovieFound_ShouldReturnFound() throws Exception {
		Movie movie = new MovieBuilder("Movie1", now,
				new PriceCategoryRegular()).id(new Long(1)).build();

		when(movieRepositoryMock.findOne(1L)).thenReturn(movie);

		mockMvc.perform(
				get("/movies/{id}", 1L).header("Accept", "application/json"))
				// .andDo(print())
	...
	@Test
	public void findById_MovieNotExisting_ShouldReturnNotFound()
			throws Exception {
		mockMvc.perform(
				get("/movies/{id}", 2L).header("Accept", "application/json"))
				.andExpect(status().isNotFound());
	}

	@Test
	public void findAll_MoviesFound_ShouldReturnFoundMovies() throws Exception {
		Movie movie1 = new MovieBuilder("Movie1", now,
				new PriceCategoryRegular()).id(new Long(1)).build();

		Movie movie2 = new MovieBuilder("Movie2", now,
				new PriceCategoryRegular()).id(new Long(2)).build();

		when(movieRepositoryMock.findAll(new Sort("id")))
...
class MovieBuilder {
	...
	public MovieBuilder(String title, Date releaseDate,
